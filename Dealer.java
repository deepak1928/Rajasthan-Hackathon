...
public class Dealer {
    ...
    Dealer(final String name, final String address, final int port, final Block root, final List<Dealer> agents) {
        this.name = name;
    ...
    void startHost() {
        executor.execute(() -> {
            try {
                ...
                listening = true;
                while (listening) {
                    final DealerServerThread thread = new DealerServerThread(Dealer.this, serverSocket.accept());
                    thread.start();
                }
                serverSocket.close();
            } catch (IOException e) {
